...
namespace EEM.Common.Clients
{
  ...
  /// </summary>
  public sealed class MessageExchangeClient : IMessageExchangeClient
  {
    /// <summary>
    ...
    public delegate void ServerRequestSentHandler(string url, IJsonRequest request);
    public event ServerRequestSentHandler OnServerRequest;
    private void ServerRequestSent(string url, IJsonRequest request)
    {
    ...
    /// <returns></returns>
    internal string ExecuteServerCommand(ServerCommand command, IJsonRequest json, bool isQueuedCommand = false)
    {
    ...
    {
      System.Diagnostics.Debug.WriteLine("Error: {0}", method);
//      MessageBox.Show(exception.Message, String.Format("Error: {0}", method));
    ...
    internal int QueueServerCommand(ServerCommand command, IJsonRequest json)
    {
