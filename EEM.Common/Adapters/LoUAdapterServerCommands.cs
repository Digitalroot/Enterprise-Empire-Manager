...
namespace EEM.Common.Adapters
{
  public partial class LoUAdapter
  {
    ...
    public IPollResponse GetCityDetails(int cityId)
    {
      var json = new JsonRequest(new
      {
        session = SessionId,
    ...
    /// <param name="cityId"></param>
    /// <returns></returns>
    public IGetPublicCityInfoResponse GetPublicCityInfo(int cityId)
    {
      var json = new JsonRequest(new
      {
        session = SessionId,
    ...
    /// <returns></returns>
    public IGetPublicPlayerInfoResponse GetPublicPlayerInfo(int playerId)
    {
      var json = new JsonRequest(new
      {
        session = SessionId,
    ...
    public string OrdersUnit(int cityId, Units[] unitTypeAndCount, string cords, AttackType attackType, DateTime? whenToSend)
    {
      ...
      var cleanUnitTypeAndCount = new Units[counter];
      int transportType = 1;
    ...
    public void SendChat(string message)
    {
      ...
      ((PollingService)PollingService).ListOfPollIds.Add(MessageExchangeClient.QueueServerCommand(ServerCommand.Poll, json));
    }
    ...
    public List<IGetPublicAllianceMemberListResponse> GetPublicAllianceMemberList(int allianceId)
    {
      var json = new JsonRequest(new
      {
        session = SessionId,
      ...
      return JsonConvert.DeserializeObject<List<IGetPublicAllianceMemberListResponse>>(result);
    }
    ...
    /// </summary>
    /// <returns></returns>
    public List<IAllianceGetRangeResponse> GetListOfAlliances()
    {
      ...
      var returnValue = JsonConvert.DeserializeObject<List<AllianceGetRangeResponse>>(result);
      return returnValue.ToList<IAllianceGetRangeResponse>();
    }
