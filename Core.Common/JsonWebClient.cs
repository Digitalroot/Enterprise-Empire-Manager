...
namespace Core.Common
{
  ...
  /// </summary>
  public class JsonWebClient : CookieAwareWebClient, IJsonWebClient
  {
    ...
    ~JsonWebClient()
    {
      ...
      using (var writer = new StreamWriter(_cookieFile, false))
      {
        ...
        {
          writer.WriteLine((string) JsonConvert.SerializeObject(cookie));
        }
        writer.Close();
      }
    }
    ...
    /// <returns></returns>
    public string QueryUrlAsJSONRequest(string url, IJsonRequest json)
    {
    ...
    /// <returns></returns>
    public string QueryUrlAsJSONRequest(string url, IJsonRequest json, string referrer)
    {
      ...
      try
      {
        ...
        writer.Write((string) json.ToString());
        writer.Close();
      }
